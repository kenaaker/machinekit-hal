...
int halcmd_startup_uuid(int quiet, char *uri, const char *svc_uuid) {
    ...
    if ((retval = rtapi_connect(rtapi_instance, uri, svc_uuid))) {
        if (!quiet) {
            fprintf(stderr, "halcmd: Can't connect to rtapi_app: %d (uri=%s uuid=%s): %s\n\n", retval, uri ? uri : "",
                    svc_uuid, rtapi_rpcerror());
...
static int parse_cmd1(char **argv) {
    struct halcmd_command *command =
        bsearch(argv[0], halcmd_commands, halcmd_ncommands, sizeof(struct halcmd_command), compare_command);
    ...
    if (!command) {
        ...
        int result = -EINVAL;
        int is_optional = command->type & A_OPTIONAL, is_plus = command->type & A_PLUS, nargs = command->type & 0xff,
            posargs;
        ...
        if (posargs > nargs && !is_plus) {
            halcmd_error("%s requires %s%d arguments, %d given\n", command->name, is_optional ? "at most " : "", nargs,
                         posargs);
            return -EINVAL;
        }
        ...
        case A_ONE | A_PLUS: {
            int (*f)(char *arg, char **rest) = (int (*)(char *, char **))command->func;
            result = f(ARG(1), REST(2));
            break;
        }

        case A_TWO: {
            int (*f)(char *arg, char *arg2) = (int (*)(char *, char *))command->func;
            result = f(ARG(1), ARG(2));
            break;
        }

        case A_TWO | A_PLUS: {
            int (*f)(char *arg, char *arg2, char **rest) = (int (*)(char *, char *, char **))command->func;
            result = f(ARG(1), ARG(2), REST(3));
            break;
        }

        case A_THREE: {
            int (*f)(char *arg, char *arg2, char *arg3) = (int (*)(char *, char *, char *))command->func;
