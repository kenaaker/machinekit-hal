...
static int mksock(int type) {
    ...
    if (fd != -1) {
        if ((flags = fcntl(fd, F_GETFL)) == -1 || fcntl(fd, F_SETFL, flags | O_NONBLOCK) == -1 ||
            (flags = fcntl(fd, F_GETFD)) == -1 || fcntl(fd, F_SETFD, flags | FD_CLOEXEC) == -1) {
            close(fd);
            fd = -1;
        }
    }
...
void log_write_async(void) {
    ...
    while (entries) {
        ...
        if ((rc = send(log_fd, entries->payload + entries->offset, entries->length, MSG_NOSIGNAL)) != -1) {
            entries->length -= rc;
            entries->offset += rc;
            connection_good = 1;
        ...
        if (errno == ECONNREFUSED || errno == ENOTCONN || errno == EDESTADDRREQ || errno == ECONNRESET) {
            ...
            if (errno == ENOENT || errno == EALREADY || errno == ECONNREFUSED || errno == EISCONN || errno == EINTR ||
                errno == EAGAIN) {
    ...
    fail:
        ...
        if ((log_opts & LOG_CONS) && (fd = open(_PATH_CONSOLE, O_WRONLY | O_NONBLOCK, 0)) != -1) {
